...
struct MangaDetailView: View {
    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItemGroup(placement: .navigationBarTrailing) {
                if let manga = manga {
                    ...
                    Button {
                        if store.isInLibrary(manga) { store.removeFromLibrary(manga) }
                        else { store.addToLibrary(manga) }
                    } label: {
                    ...
                    Menu {
                        Button {
                            if store.isWantToRead(manga) { store.removeWantToRead(manga) }
                            else { store.addWantToRead(manga) }
                        } label: {
                        ...
                        Button {
                            if store.isCompleted(manga) { store.removeCompleted(manga) }
                            else { store.addCompleted(manga) }
                        } label: {
        ...
        .fullScreenCover(item: $selectedChapter) { chapter in
            if let manga = manga {
                let progress = store.history.first { $0.mangaSlug == manga.slug && $0.chapterSlug == chapter.slug }
                ...
                ZStack {
                    ...
                    VStack(spacing: 20) {
                        ...
                        Text("Chapter data missing").foregroundColor(.white)
                        Button("Close") { selectedChapter = nil }.foregroundColor(ZTheme.accent)
                    }
                }
            }
        }
    ...
    var loadingState: some View {
        VStack(spacing: 20) {
            HStack(alignment: .top, spacing: 16) {
                RoundedRectangle(cornerRadius: 12).fill(ZTheme.card).frame(width: 110, height: 155)
                VStack(alignment: .leading, spacing: 8) {
                    Text(preloadTitle.isEmpty ? "Loading..." : preloadTitle)
                        .font(.system(size: 18, weight: .bold)).foregroundColor(ZTheme.textPrimary)
    ...
    var errorState: some View {
        VStack(spacing: 12) {
            Image(systemName: "exclamationmark.triangle")
                .font(.system(size: 40, weight: .ultraLight)).foregroundColor(ZTheme.danger)
            Text(errorMessage ?? "Failed to load")
                .foregroundColor(ZTheme.textSecondary).multilineTextAlignment(.center).padding(.horizontal)
            Button("Retry") { Task { await loadDetail() } }.foregroundColor(ZTheme.accent)
        }
    }
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                if multiSelectMode {
                    HStack {
                        ...
                        .buttonStyle(.borderedProminent).tint(ZTheme.accent).disabled(selectedChapters.isEmpty)
                        Spacer()
                    }
                ...
                LazyVStack(spacing: 0) {
                    ForEach(sortedChapters) { chapter in
                        ChapterRow(
                            ...
                            action: {
                                if multiSelectMode {
                                    if selectedChapters.contains(chapter.slug) { selectedChapters.remove(chapter.slug) }
                                    else { selectedChapters.insert(chapter.slug) }
                                } else {
                                    guard !manga.chapters.isEmpty else { showChapterError = true; return }
    ...
    func heroSection(manga: Manga) -> some View {
        HStack(alignment: .top, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                Text(manga.title).font(.system(size: 17, weight: .bold)).foregroundColor(ZTheme.textPrimary).lineLimit(3)
                if !manga.author.isEmpty { Text(manga.author).font(.system(size: 13)).foregroundColor(ZTheme.textSecondary) }
                HStack(spacing: 6) {
                    if !manga.status.isEmpty { StatusBadge(text: manga.status) }
                    if !manga.rating.isEmpty {
                        HStack(spacing: 3) {
                            Image(systemName: "star.fill").font(.system(size: 10)).foregroundColor(ZTheme.accent)
                            Text(manga.rating).font(.system(size: 12, weight: .medium)).foregroundColor(ZTheme.textSecondary)
                        }
                    }
                }
                if !manga.genres.isEmpty {
                    FlowLayout(spacing: 5) {
                        ForEach(manga.genres, id: \.self) { genre in
                            Text(genre).font(.system(size: 10, weight: .medium)).foregroundColor(ZTheme.accent)
                                .padding(.horizontal, 8).padding(.vertical, 3).background(ZTheme.accentDim).clipShape(Capsule())
                        }
                    }
                }
                if let firstChapter = manga.chapters.min(by: { (Double($0.number) ?? 0) < (Double($1.number) ?? 0) }) {
                    Button {
                        ...
                        } else { selectedChapter = firstChapter }
                    } label: {
                        HStack(spacing: 6) {
                            Image(systemName: "play.fill").font(.system(size: 11))
                            Text(store.history.first(where: { $0.mangaSlug == manga.slug }) != nil
                                 ? "Continue Ch.\(store.history.first(where: { $0.mangaSlug == manga.slug })!.chapterNumber)"
                                 : "Start Reading")
                                .font(.system(size: 13, weight: .semibold))
                        }
                        .foregroundColor(ZTheme.bg).padding(.horizontal, 16).padding(.vertical, 8)
                        .background(ZTheme.accent).clipShape(RoundedRectangle(cornerRadius: 8))
                    }
                } else {
                    Text("No chapters available").font(.system(size: 13)).foregroundColor(ZTheme.textSecondary).padding(.vertical, 8)
                }
            }
            Spacer()
        }
    ...
    func descriptionSection(_ text: String) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Synopsis").font(.system(size: 11, weight: .semibold)).foregroundColor(ZTheme.textSecondary).tracking(2)
                .padding(.horizontal, 20)
            Text(text).font(.system(size: 14)).foregroundColor(ZTheme.textSecondary).lineSpacing(4).padding(.horizontal, 20)
        }
    }

    func chaptersHeader(count: Int) -> some View {
        HStack {
            Text("\(count) CHAPTERS").font(.system(size: 11, weight: .semibold)).foregroundColor(ZTheme.textSecondary).tracking(2)
            Spacer()
            Button {
                withAnimation(.easeInOut(duration: 0.2)) { chapterSortAsc.toggle() }
            } label: {
                HStack(spacing: 4) {
                    Image(systemName: chapterSortAsc ? "arrow.up" : "arrow.down").font(.system(size: 11))
                    Text(chapterSortAsc ? "Oldest" : "Newest").font(.system(size: 12))
                }
                .foregroundColor(ZTheme.textSecondary)
            }
        }
    ...
    func loadDetail() async {
        ...
        if let cached = store.mangaCache[slug] {
            await MainActor.run { manga = cached; isLoading = false }
            return
        }
        do {
            let m = try await MangaService.shared.fetchDetail(slug: slug)
            await MainActor.run { manga = m; store.cacheManga(m); isLoading = false }
...
struct ChapterRow: View {
    ...
    var body: some View {
        Button(action: action) {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    HStack(spacing: 6) {
                        ...
                        if let p = progress {
                            Text("· p.\(p.pageIndex + 1)").font(.system(size: 12)).foregroundColor(ZTheme.accent)
                        }
                        if isDownloaded {
                            Image(systemName: "arrow.down.circle.fill").foregroundColor(ZTheme.success).font(.system(size: 12))
                        } else if isDownloading {
                            ProgressView().scaleEffect(0.7).tint(ZTheme.accent)
                        }
                    }
                    if !chapter.date.isEmpty {
                        Text(chapter.date).font(.system(size: 11)).foregroundColor(ZTheme.textTertiary)
                    }
                }
                Spacer()
                if !multiSelectMode {
            ...
            .padding(.horizontal, 20).padding(.vertical, 14).background(ZTheme.bg)
        }
        .contextMenu {
            if !multiSelectMode {
                Button { markReadAction(!isRead) } label: {
                    Label(isRead ? "Mark as Unread" : "Mark as Read", systemImage: isRead ? "eye.slash" : "eye")
                }
                if isDownloaded {
                    Button(role: .destructive, action: deleteDownloadAction) { Label("Delete Download", systemImage: "trash") }
                } else if !isDownloading {
                    Button(action: downloadAction) { Label("Download Chapter", systemImage: "arrow.down.circle") }
                } else {
                    Label("Downloading...", systemImage: "hourglass").disabled(true)
                }
            }
        }
    }
...
struct StatusBadge: View {
    ...
    var color: Color {
        text.lowercased().contains("مستمر") || text.lowercased().contains("ongoing")
            ? Color(hex: "#4CAF82") : ZTheme.textTertiary
    }
    var body: some View {
